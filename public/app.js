  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    if (registerButton) {
      registerButton.addEventListener('click', async (e) => {
        ...
        const response = await saveUser(email, password);
    ...
    window.handleDelete = window.deleteTask = async function(taskId) {
      const currentLang = localStorage.getItem('site_lang') || 'en';
      // Простая проверка языка для confirm
      const msg = currentLang === 'ua' || currentLang === 'uk' ? 'Видалити цю задачу?' : 'Delete this task?';
      ...
      try {
        ...
        if (res.ok) {
          ...
          // Удаляем из календаря (если открыт)
          const calendarTask = document.querySelector(`.date-task-card[data-task-id="${taskId}"]`);
          if (calendarTask) calendarTask.remove();

          removeAllMenus();
          showNotification('Deleted / Видалено', 'success');
          
          // Если нужно обновить календарь точек
    ...
    window.handleDuplicate = async function(taskId) {
      // Нам нужно получить данные оригинальной задачи.
      // Так как у нас нет локального массива tasks, возьмем данные из DOM строки или сделаем fetch.
      // Для простоты, давай сделаем быстрый fetch списка, найдем задачу и скопируем.
      
      const userData = await getCurrentUserData(); // Твоя функция возвращает объект с userId
      if (!userData) return;

      // Получим актуальный список задач (или можно хранить кэш в глобальной переменной)
      const tasksRes = await fetch(`/api/tasks?email=${encodeURIComponent(getCurrentUser())}`);
      const data = await tasksRes.json();
      const originalTask = data.tasks.find(t => t.id == taskId); // == т.к. id может быть строкой или числом

      if (!originalTask) return;

      // Создаем копию через saveTask (который делает POST запрос)
      const newTaskPayload = {
        title: originalTask.name + ' (copy)', // Помни, в saveTask ты шлешь title
      ...
      // Вызываем твой обновленный saveTask
      await saveTask(newTaskPayload);
      removeAllMenus();
    };
